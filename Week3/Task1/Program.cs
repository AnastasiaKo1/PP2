...
namespace ConsoleApp24
{
    class FarManager//создаю класс Farmanager 
    {
        ...
        public void Show(DirectoryInfo dire, int z)//метод в который мы передаем две данные: инф о папке и курсор
        {
        ...
        public void Start(string path)//функцию в которой распределяю кнопки 
        {
            ...
            while (button.Key != ConsoleKey.Escape)// это программа будет работать пока не нажать esc
            {
                ...
                Show(dir, cursor);// вызываю метод show
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            far.Start(@"C:\Users\asus\Desktop\pp2\Новая папка"); //указываем путь в котором мы будем запускать FarManager
        }
    }
}
